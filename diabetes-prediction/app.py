...
def load_model_and_scaler():
    try:
        ...
        scalar = joblib.load('diabetes-prediction/scalar_svm.pkl')
        return model, scalar
...
st.sidebar.title("Patient Informations")
...
if predict_btn:
    ...
    input_std = scalar.transform(input_data)
    prediction = model.predict(input_std)[0]

    try:
        probability = model.predict_proba(input_std)[0]
        prob_neg = probability[0] * 100
        prob_posve = probability[1] * 100

    except:
        prob_posve = 100 if prediction == 1 else 0
        prob_neg = 100 - prob_posve
    ...
    with col1:
        ...
        pcol1.metric("Non-Diabetic", f"{prob_neg:.1f}%")
        pcol2.metric("Diabetic", f"{prob_posve:.1f}%")
